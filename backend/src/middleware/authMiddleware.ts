 ...
 * Checks if the authenticated user is the admin of the specified team
 * 
 * This middleware:
 * 1. Requires authentication (user must be logged in)
 * 2. Checks if user is either a global admin OR the team's admin
 * 3. Used for team modification operations (update, delete, add/remove members)
 * 
 * Usage:
 * router.put('/teams/:id', requireAuth, requireTeamOwnership, updateTeam);
 * 
 * @throws AuthenticationError if user is not authorized to manage this team
 */
export const requireTeamOwnership = async (
    ...
    try {
        ...
        // Check if user is the admin of this specific team
        // Note: This assumes Team model has adminId field
        // Will be fully implemented when Team CRUD operations are added
        ...
        if (team.adminId.toString() !== (req.user as any)._id.toString()) {
            throw new AuthenticationError('Only the team admin can perform this action');
        }
